...
| SMSgate / Gammu / SIM800/SIM900 / RASP-IVR / Verboice / VBVoice | **Partial** | Yes (on-prem hardware) | A `gammu` container with `gammu` + `gammu-smsd` is built from `services/gammu/Dockerfile`; `GAMMU_COMMAND`/`SMSGATE_COMMAND` defaults target the sibling container. A `verboice` container (with a MySQL 5.7 `verboice-db`) is built from `services/verboice/Dockerfile`; `call_flow_design` can create real Verboice call flows when `VERBOICE_COMMAND` is configured. `ivr_hardware_setup`, `call_flow_design`, and `dtmf_handler` use configured `RASP_IVR_COMMAND`, `VERBOICE_COMMAND`, and `VBVOICE_COMMAND` hooks. `campaign_orchestrate` can use any configured IVR command hook. All fall back to the modeled simulator. Real SMS/calls need a GSM modem or a SIP/PSTN channel in Verboice. |
...
| `call_flow_design` | Call Flow Designer | Verboice | offline-only | Uses `VERBOICE_COMMAND` hook to create real Verboice call flows when configured; otherwise modeled call tree. |
