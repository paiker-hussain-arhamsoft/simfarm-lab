...
| SMSgate / Gammu / SIM800/SIM900 / RASP-IVR / Verboice / VBVoice | **Partial** | Yes (on-prem hardware) | A `gammu` container with `gammu` + `gammu-smsd` is built from `services/gammu/Dockerfile`; `GAMMU_COMMAND`/`SMSGATE_COMMAND` defaults target the sibling container. `modem_topology`, `smsgate_config`, `modem_control`, `sim_provision_plan`, `sim_activate`, `carrier_access`, and `sms_send` use those hooks when configured. `ivr_hardware_setup`, `call_flow_design`, and `dtmf_handler` use configured `RASP_IVR_COMMAND`, `VERBOICE_COMMAND`, and `VBVOICE_COMMAND` hooks. `campaign_orchestrate` can use any configured IVR command hook. All fall back to the modeled simulator. Real SMS/calls need a GSM modem and `devices` override in compose. |
