...
| ElizaOS / Botpress / LangGraph (framework only) / Socioboard | **Partial** | Yes (self-hosted) | `persona_design`, `behavior_model`, and `fleet_orchestrate` can use LangGraph (in-process LLM graph), ElizaOS, or Botpress. `tool='langgraph'|'botpress'|'elizaos'` forces a provider; otherwise the first configured available provider is used. All fall back to modeled stubs. Socioboard remains a stub. |
