...
| ElizaOS / Botpress / LangGraph (framework only) / Socioboard | **Partial** | Yes (self-hosted) | `persona_design`, `behavior_model`, and `fleet_orchestrate` are wired to the ElizaOS REST API when `ELIZAOS_URL` is set and to the Botpress v12/cloud API when `BOTPRESS_URL` is set; both fall back to modeled stubs. `tool='botpress'` forces the Botpress provider. LangGraph is installed; Socioboard remains a stub. |
