...
| Nmap / OpenVAS / OWASP ZAP / Burp Suite / Metasploit / CAI (Alias Robotics) | **Partial** | Mixed | `nmap_scan` is wired to the system `nmap` binary and `openvas_scan` is wired to the `immauss/openvas` container via `gvm-cli` for authorized targets; both fall back to stubs. ZAP/Burp/Metasploit/CAI remain simulated. |
