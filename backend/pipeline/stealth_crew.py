"""TIER 4 simulated stealth detection pipeline."""
import json, uuid
from collections.abc import AsyncGenerator
from langchain_core.messages import HumanMessage, SystemMessage
from backend.agents.stealth_crew import STEALTH_CREW, CrewAgentDef
...
def _tool_params(t,inp):
    base={"evasion_target":inp.get("evasion_target",""),"browser_engine":inp.get("browser_engine",""),"scenario":inp.get("scenario",""),"objective":inp.get("objective","")}
    return base
async def _run_agent_tools(agent: CrewAgentDef,inp):
    ...
    for tid in agent.tools:
        ...
        params=_tool_params(tid,inp); out.append({"type":"tool_call","worker":agent.id,"tool":tid,"provider":spec.provider,"params":params,"attempt":1})
        try: out.append({"type":"tool_result","worker":agent.id,"tool":tid,"ok":True,"result":await registry.call_tool(tid,**params)})
...
async def run(inp,session_id,backend: LLMBackend)->AsyncGenerator[str,None]:
    rid=str(uuid.uuid4());_active[rid]=True;yield _sse({"type":"pipeline_start","run_id":rid,"model":backend.value,"framework":"stealth_crew","scenario":inp.get("scenario","")});history=[];llm=create_langchain_llm(backend)
    try:
        for a in STEALTH_CREW:
        ...
        memory.save_run(session_id,rid,f"[stealth] {inp.get('objective','')}",history);yield _sse({"type":"done","run_id":rid})
    except Exception as e:yield _sse({"type":"error","message":f"Stealth error: {e}"})
    finally:_active.pop(rid,None)
async def route(inp,session_id,backend=""):
    r=compliance.record(session_id,"stealth.plan",f"[stealth] {inp.get('objective','')} | scenario={inp.get('scenario','')}",inp,inp.get("authorized",False),authorization_ref=inp.get("authorization_ref",""),approver=inp.get("approver",""))
    ...
    if not r.allowed:
        yield _sse({"type":"compliance_block","audit_id":r.audit_id,"message":"This request was flagged by the safety screen and cannot proceed as-is. It has been recorded in the audit log (id "+r.audit_id+") for legal review.","reasons":r.reasons});yield _sse({"type":"done","blocked":True});return
...
async def run_demo(inp,session_id):
    rid=str(uuid.uuid4());yield _sse({"type":"pipeline_start","run_id":rid,"model":"demo-mode","demo":True,"framework":"stealth_crew","scenario":inp.get("scenario","")});history=[]
    for a in STEALTH_CREW:
        for e in await _run_agent_tools(a,inp):yield _sse(e)
        yield _sse({"type":"agent_start","agent":a.id,"role":a.role,"framework":a.framework,"color":a.color,"icon":a.icon,"description":a.description,"tools":a.tools})
        c=f"## {a.role}\n1. Simulated {a.framework} model for {inp.get('scenario','')} and \"{inp.get('objective','')}\".\n\n## Detection & Countermeasures\nMonitor fingerprint, WAF, timing, network and storage signals; no real evasion occurs.\n\n## Compliance Checklist\n☑ Lab-only ☑ simulation-only ☑ audit-logged."
    ...
    memory.save_run(session_id,rid,f"[stealth] {inp.get('objective','')}",history);yield _sse({"type":"done","run_id":rid,"demo":True})
